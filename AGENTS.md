...
- Live-provider behavior (e.g. DeepSeek eliding `reasoning_content`) is **not an SDK contract** — probe softly, assert only what the SDK guarantees (call success, parsing, canonical finish). Model answer correctness is never an assertion. Elision is absorbed by contract rather than hope: on a tool-bearing request, providers flagged `Quirks.EchoReasoningWithTools` always get the `reasoning_content` key, empty included.
